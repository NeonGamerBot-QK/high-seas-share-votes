...
async function main() {
    ...
    button.addEventListener('click', async (e) => {
       ...
       const matchupRepoUrl =repo_url
        ...
        const anon = await chrome.storage.sync.get("anon").then(d=>d.anon) || false
        const send_it_to_user = await chrome.storage.sync.get("send_to_user").then(d=>d.send_to_user) || false
...
if(window.location.pathname.includes("/wonderdome")) {
    Array.from(document.querySelectorAll("#vote-button")).filter(d=>!d.getAttribute("data-touched")).forEach(d => {
        ...
        d.addEventListener('click', async (e) => {
            ...
            matchup_title = d.parentElement.parentElement.parentElement.querySelector(`h2[class="font-heading text-2xl font-semibold text-indigo-600 dark:text-indigo-300 mb-4`).innerText.replace(`\nThis is a project update`, ``).trim()
            matchup_author = d.parentElement.parentElement.parentElement.querySelector(`span[class="inline-flex items-center gap-1 rounded-full px-2 border text-sm leading-none text-purple-600 bg-purple-50 border-purple-500/10 "`).parentElement.href.split("?channel=")[1]
...
window.onload = async () => {
    const isAuthed = await chrome.storage.sync.get('is_authed').then(d=>d.is_authed)    
    if(!isAuthed) {
        alert(`You are not authed! please go to the options page for the chrome extension "High Seas Share Votes"`)
